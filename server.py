...
API = os.getenv('ANIME_API_URL', 'https://nex-series-server-2.vercel.app')
...
# ============================================================
# /api/secure/pipe — decrypt AES-GCM payload, proxy via /proxy/hls
# HOW IT WORKS:
#   1. The server injects SESSION_AES_B64 into every HTML page as
#      <meta name="k-session">. The frontend JS reads that key.
#   2. The frontend encrypts {url, ref} as AES-GCM and sends it here
#      as the ?e= query param.
#   3. We decrypt it locally using SESSION_AES_KEY and redirect to
#      the existing /proxy/hls endpoint which rewrites HLS manifests,
#      sets correct referer/origin headers, and streams the content.
# ============================================================
@app.route('/api/secure/pipe', methods=['GET'])
def secure_pipe():
    """Decrypt the AES-GCM 'e' payload and proxy the target stream."""
    from urllib.parse import urlencode, quote

    e_b64 = request.args.get('e', '').strip()
    if not e_b64:
        return jsonify({"error": "Missing 'e' parameter"}), 400

    # Step 1: decrypt using our local session key (not a remote server)
    try:
        raw = decrypt_payload(e_b64)
    except Exception as ex:
        print(f"[PIPE] Decryption failed: {ex}")
        return jsonify({"error": "Invalid or tampered payload"}), 400

    # Step 2: parse the decrypted bytes as JSON {url, ref?, headers?}
    # Fall back to treating the whole thing as a plain URL string.
    try:
        data = json.loads(raw.decode('utf-8'))
        if isinstance(data, str):
            data = {'url': data}
    except Exception:
        data = {'url': raw.decode('utf-8', errors='replace').strip()}

    target_url = (
        data.get('url') or data.get('stream') or
        data.get('src') or data.get('href') or ''
    ).strip()
    ref = (data.get('ref') or data.get('referer') or '').strip()

    if not target_url or not target_url.startswith('http'):
        print(f"[PIPE] No valid URL in decrypted payload: {data}")
        return jsonify({"error": "No valid URL in payload"}), 400

    print(f"[PIPE] Proxying: {target_url[:80]}... ref={ref[:40] if ref else 'auto'}")

    # Step 3: redirect to existing /proxy/hls which handles HLS manifest
    # rewriting, correct referer/origin headers, and byte-range streaming.
    params = {'url': target_url}
    if ref:
        params['ref'] = ref
    from flask import redirect
    return redirect('/proxy/hls?' + urlencode(params), code=302)
...
WORKER_API = os.getenv('WORKER_API', 'https://nex-series-server-2.vercel.app')
STREAM_PROXY = os.getenv('STREAM_PROXY', 'https://nex-series-server-2.vercel.app')
CDN_BASE = os.getenv('CDN_BASE', 'https://nex-series-server-2.vercel.app')
CDN_REFERER = os.getenv('CDN_REFERER', 'https://nex-series-server-2.vercel.app')
