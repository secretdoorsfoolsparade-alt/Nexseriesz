...
# ============================================================
# FIXED: /api/secure/pipe - Now forwards to Vercel API
# ============================================================
@app.route('/api/secure/pipe', methods=['GET'])
def secure_pipe():
    """Forward secure pipe requests to Vercel API."""
    import requests
    ...
    try:
        # Forward to your Vercel API
        resp = requests.get(
            f"https://nex-series-server-2.vercel.app/api/secure/pipe?e={e_b64}",
            headers={
...
if __name__ == '__main__':
    ...
    app.run(host=host, port=port, debug=False)        uid, cv = decoded.split(':', 1)
        return uid or None, cv
    except Exception:
        return None, None
...
if __name__ == '__main__':
    ...
    host = os.getenv('HOST', '0.0.0.0')
    print(f'\n  Kurovexa -> http://{host}:{port}\n')
    app.run(host=host, port=port, debug=False)        return None
    a = accept.lower()
    if entry.get('br') is not None and 'br' in a:
        return 'br'
    if entry.get('gzip') is not None and 'gzip' in a:
